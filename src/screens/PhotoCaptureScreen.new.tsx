  ...
  const handleScannedIdSubmit = async (scannedId: string) => {
    ...
    try {
      ...
      const newBatch: Partial<PhotoBatch> = {
        ...
        status: 'InProgress', // Changed from 'active' to match expected enum
        syncStatus: 'pending',
        photoCount: 0
      };
      ...
      if (batchId) {
        const createdBatch = await getBatchDetails(batchId);
  ...
  const processAndSavePhoto = async (photo: CameraCapturedPicture, isDefect: boolean) => {
    ...
    try {
      ...
      const metadata: PhotoMetadata = {
        ...
        // Add any other required metadata fields here
      };
      ...
      if (currentBatch) {
        const photoData: PhotoData = {
          ...
          metadata,
          syncStatus: 'pending',
          isDefect // Add this property if it's part of PhotoData
        };
